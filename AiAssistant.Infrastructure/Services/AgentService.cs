...
public sealed class AgentService : IAgentService
{
    ...
    private static string ExecuteCheckDocumentExpiry(IReadOnlyList<DocumentChunk> chunks)
    {
        ...
        return JsonSerializer.Serialize(expiryResults);
    }
    ...
    private async Task<string> ExecuteGetAccountStatementAsync(string inputJson, CancellationToken ct)
    {
        ...
        sb.AppendLine(
            $"Página {statement.PageNumber} de {statement.TotalPages} | Total registros: {statement.PageCount}");
        sb.AppendLine();
        ...
        foreach (var tx in statement.Data)
        {
            ...
            sb.AppendLine(
                $"  Débito: {tx.DebitAmount:N2} | Crédito: {tx.CreditAmount:N2} | Balance: {tx.RunningBalance:N2}");
        ...
        try
        {
            ...
            var messages = new List<Message>
            {
                ...
                new()
                {
                    ...
                    Content = llmResponse.ToolUseRequests.Select(t => (ContentBase)new ToolUseContent
                    {
                        Id = t.ToolUseId,
                        Name = t.ToolName,
                        Input = JsonNode.Parse(t.InputJson)
                    }).ToList()
                },
                new()
                {
                    Role = RoleType.User,
                    Content = toolResults.Select(kvp => (ContentBase)new ToolResultContent
                    {
                        ToolUseId = kvp.Key,
                        Content = new List<ContentBase> { new TextContent { Text = kvp.Value } }
                    }).ToList()
                }
            };
